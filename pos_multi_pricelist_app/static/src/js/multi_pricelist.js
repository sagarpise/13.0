odoo.define('pos_multi_pricelist_app.multi_pricelist', function(require) {
	...
	var BaseWidget1 = PosBaseWidget.include({
	...
	var pricelist_button = screens.set_pricelist_button.include({
				...
				confirm: function (pricelist) {
					...
					for (var i = 0; i < self.pos.currencies.length; i++) {
						if(self.pos.currencies[i].id == pricelist.currency_id[0]){
							...
							new_currency = self.pos.currencies[i];
							break;
						}
					}
					...
					self.pos.currency = new_currency;
	...
	models.Order = models.Order.extend({
		...
		init_from_JSON: function(json) {
			...
			var have = config_currencies.includes(json.new_currency.id);
